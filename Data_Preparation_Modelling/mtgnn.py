...
class MixProp(nn.Module):
    r"""A modified MixProp implementation that's flexible with arguments"""

    def __init__(self, c_in, c_out, gdep, dropout, alpha, *args):
    ...
    def forward(self, X: torch.FloatTensor, A: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        # Ensure A is the right shape
        if A.shape[0] != X.shape[2]:
            print(f"Fixing A shape: {A.shape} to match nodes: {X.shape[2]}")
            A = torch.ones(X.shape[2], X.shape[2]).to(X.device)
...
class MTGNNLayer(nn.Module):
    ...
    def __init__(
        ...
        if gcn_true:
            print("🔍 Checking MixProp signature:", inspect.signature(MixProp))
...
class MTGNN(nn.Module):
    ...
    def forward(
        ...
        if self._seq_length < self._receptive_field:
            ...
            # Check and fix adjacency matrix
            num_nodes = X_in.size(2)
            if A_tilde is not None and A_tilde.shape != (num_nodes, num_nodes):
                print(f"MTGNN: Fixing adjacency matrix. Expected shape ({num_nodes}, {num_nodes}), got {A_tilde.shape}")
                if os.path.exists("correct_adj.pt"):
                    A_tilde = torch.load("correct_adj.pt").to(X_in.device)
                else:
                    A_tilde = torch.ones((num_nodes, num_nodes)).to(X_in.device)
